...
svReceiverLoop(void* threadParameter)
{
    ...
    while (self->running) {
    	// SVReceiver_tick(self, 1, 1) - two ones passed to that function are dummy parameters
        if (SVReceiver_tick(self, 1, 1) == false)
            Thread_sleep(1);
    }
...
bool
SVReceiver_tick(SVReceiver self, uint8_t *buffer, uint16_t len)
{
    //int packetSize = Ethernet_receivePacket(self->ethSocket, self->buffer, ETH_BUFFER_LENGTH);
	self->buffer = buffer;

    if (len > 0) {
        parseSVMessage(self, len);
        return true;
    }
    else
        return false;
...
SVClientASDU_getINT32(SVClientASDU self, int index)
{
    ...
    //int32_t retVal;
    //memcpy(retVal, self->dataBuffer+index, 4);
...
SVClientASDU_getINT64U(SVClientASDU self, int index)
{
	...
	//uint64_t retVal;
    //memcpy(retVal, self->dataBuffer+index, 8);
